...
class Scraper {
	...
	createNewRequest(resource) {
		...
		const requestPromise = Promise.resolve()
			...
			.then(async function requestCompleted(responseData) {
				...
				if (!urlsEqual(responseData.url, url)) {
	...
	async load() {
		const loadResourcePromises = this.resources.map((resource) => this.requestResource.bind(this, resource));
	...
	async waitForLoad() {
		...
		if (!loadingIsFinished) {
			const saveResourcePromises = resourcesToSave.map((resource) => this.saveResource.bind(this, resource));
